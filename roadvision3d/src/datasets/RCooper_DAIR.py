...
class RCooper_DAIR(data.Dataset):
    def __init__(self, split, cfg):
        ...
        self.depth_dir = os.path.join(self.data_dir)
        self.calib_dir = os.path.join(self.data_dir)
    ...
    def get_label(self, idx):
        label_file = os.path.join(self.data_dir, idx + '.json').replace("image", "label/virtuallidar")
        assert os.path.exists(label_file)
        return get_objects_from_label(label_file, self.get_calib(idx))

    def get_calib(self, idx):
        camera_calib_file = os.path.join(self.data_dir, idx + '.json').replace("image", "calib/camera_intrinsic")
        lidar_calib_file = os.path.join(self.data_dir, idx + '.json').replace("image", "calib/virtuallidar_to_camera")
...
def get_objects_from_label(label_file, calib):
    ...
    with open(label_file, 'r') as f:
        data = json.load(f)
    objects = [Object3d.from_RCooper_dair_json(obj, calib) for obj in data]
    return objects