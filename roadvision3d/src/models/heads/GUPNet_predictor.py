...
from roadvision3d.src.models.losses.loss_function import extract_input_from_tensor
from roadvision3d.src.engine.decode_helper import _topk,_nms
...
class GUPNetPredictor(nn.Module):
    def __init__(self, cfg, channels, first_level):
        ...
        self.mean_size = nn.Parameter(torch.tensor(cfg['dataset']['cls_mean_size'],dtype=torch.float32),requires_grad=False)
    ...
    def forward(self, feat, calibs, coord_ranges, targets=None, mode='train'):
        ...
        if mode=='train':   #extract train structure in the train (only) and the val mode
            ...
            inds,cls_ids = _topk(_nms(torch.clamp(ret['heatmap'].sigmoid(), min=1e-4, max=1 - 1e-4)), K=50)[1:3]
...
def build_GUPNet_predictor(cfg, in_channels, first_level):
    return GUPNetPredictor(cfg, in_channels, first_level)