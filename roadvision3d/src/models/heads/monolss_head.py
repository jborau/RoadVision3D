...
class MonoLSSHead(nn.Module):
    ...
    def forward(self, features, calib, targets=None, coord_ranges=None, epoch=1, mode='train', calibs_tmp=None, info=None, cls_mean_size=None):
        ...
        if mode == 'train':
            ...
            result = post_processor(x, calibs_tmp, coord_ranges, info=info, cls_mean_size=cls_mean_size)
            return result, {}
        # if self.training:
...
def post_processor(outputs, calib_tmp, coord_ranges, info, cls_mean_size):
