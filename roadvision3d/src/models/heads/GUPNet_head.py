...
from .GUPNet_predictor import build_GUPNet_predictor
from roadvision3d.src.models.losses.GUPNet_loss import build_GUPNet_loss
from roadvision3d.src.models.losses.GUPNet_postprocessor import build_GUPNet_postprocessor

from roadvision3d.src.models.losses.loss_function import LSS_Loss
import torch
import torch.nn as nn

class GUPNetHead(nn.Module):
    def __init__(self, cfg, in_channels, first_level):
        super(GUPNetHead, self).__init__()

        self.predictor = build_GUPNet_predictor(cfg, in_channels, first_level)
        self.loss_evaluator = build_GUPNet_loss(cfg)
        self.post_processor = build_GUPNet_postprocessor(cfg)
    ...
    def forward(self, features, calib, targets=None, coord_ranges=None, epoch=1, mode='train', info=None):
        ...
        if mode=='train':
            ...
            result = post_processor(x, calib, info=info, cls_mean_size=self.cls_mean_size)
            # result = x
            return result, {}


def build_GUPNet_head(cfg, in_channels, first_level):
    return GUPNetHead(cfg, in_channels, first_level)
